...
var Api = inherit({
    ...
    generateHelp: function (format) {
        ...
        var preparedMethods = Object.keys(methods).reduce(function (result, methodName) {
            ...
            if (!method.getOption('hiddenOnDocPage')) {
    ...
    _getMethods: function (methodPathPattern) {
        ...
        return files.reduce(function (result, file) {
            ...
            if (method.getUsedDeprecatedMethods().length) {
                this._showDeprecatedWarning(method);
            }
    ...
    /**
     * Shows a deprecated warning.
     *
     * @param {ApiMethod} method
     */
    _showDeprecatedWarning: function (method) {
        var deprecatedMethods = method.getUsedDeprecatedMethods()
        ...
        console.warn(
            '\033[31m',
            '\n[BLA] Method %s uses deprecated methods: %s. They will be remove in the next version.'
                .replace('%s', method.getName())
                .replace('%s', deprecatedMethods.join(', ')),
            '\n[BLA] Please use the new ApiMethod interface. See documentation on ',
            'https://github.com/baby-loris/bla/#constructormethod.',
            '\033[0m\n'
        );
    }
});
